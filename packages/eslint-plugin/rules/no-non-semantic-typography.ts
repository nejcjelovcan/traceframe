...
import {
  ...
  getTypographySuggestion as getTypographySuggestionFromLib,
  isNonSemanticTypographyClass as isNonSemanticTypographyClassFromLib,
} from '@nejcjelovcan/traceframe-ui-library'

import type { TSESTree } from '@typescript-eslint/utils'

type MessageIds = 'nonSemanticTypography' | 'suggestSemantic'
...
function getSuggestedReplacement(className: string): string | undefined {
  return getTypographySuggestionFromLib(className)
}

export const noNonSemanticTypography = createRule<Options, MessageIds>({
  name: 'no-non-semantic-typography',
  meta: {
    type: 'suggestion',
    docs: {
      description:
        'Enforce usage of semantic typography tokens instead of arbitrary font values or oversized text',
    },
    ...
    messages: {
      nonSemanticTypography: 'Use semantic typography instead of "{{className}}". {{suggestion}}',
  ...
  create(context) {
    ...
    function checkClassNameValue(node: TSESTree.Node, value: string) {
      ...
      for (const className of classes) {
        if (isNonSemanticTypographyClassFromLib(className)) {
          ...
          context.report({
            ...
            data: {
              className,
              suggestion: suggestion || 'Use semantic typography tokens',
            },
          })
        }
      }
