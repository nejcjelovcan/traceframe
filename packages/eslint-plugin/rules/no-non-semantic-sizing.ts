...
import {
  ...
  getSizingSuggestion as getSizingSuggestionFromLib,
  isNonSemanticSizingClass as isNonSemanticSizingClassFromLib,
} from '@nejcjelovcan/traceframe-ui-library'

import type { TSESTree } from '@typescript-eslint/utils'

type MessageIds = 'nonSemanticSizing' | 'suggestSemantic'
...
function getSuggestedReplacement(className: string): string | undefined {
  return getSizingSuggestionFromLib(className)
}

export const noNonSemanticSizing = createRule<Options, MessageIds>({
  name: 'no-non-semantic-sizing',
  meta: {
    type: 'suggestion',
    docs: {
      description:
        'Enforce usage of semantic sizing tokens instead of numeric sizing values for element dimensions',
    },
    fixable: 'code',
    hasSuggestions: true,
    schema: [
    ...
    messages: {
      nonSemanticSizing:
        'Use semantic sizing token instead of "{{className}}". Consider using "{{suggestion}}" or another semantic token.',
  ...
  create(context) {
    ...
    function checkClassNameValue(node: TSESTree.Node, value: string) {
      ...
      for (const className of classes) {
        if (isNonSemanticSizingClassFromLib(className)) {
          ...
          context.report({
            ...
            data: {
              className,
              suggestion: suggestion || 'a semantic sizing token',
            },
            ...(isExactSuggestion && {
              suggest: [
                {
