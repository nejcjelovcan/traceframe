...
type MessageIds = 'nonSemanticShadow' | 'suggestSemantic'
...
export const noNonSemanticShadows = createRule<Options, MessageIds>({
  name: 'no-non-semantic-shadows',
  meta: {
    type: 'problem',
    docs: {
      description: 'Enforce usage of semantic shadow tokens instead of standard Tailwind shadows',
    },
    fixable: 'code',
    hasSuggestions: true,
    schema: [
    ...
    messages: {
      nonSemanticShadow:
        'Use semantic shadow token instead of "{{className}}". Consider using "{{suggestion}}" or another semantic token.',
  ...
  create(context) {
    ...
    function checkClassNameValue(node: TSESTree.Node, value: string) {
      ...
      for (const className of classes) {
        if (isNonSemanticShadowClass(className)) {
          const suggestion = getShadowSuggestion(className)
          ...
          context.report({
            ...
            data: {
              className,
              suggestion: suggestion || 'a semantic shadow token',
            },
            ...(suggestion && {
              suggest: suggestion.split(' or ').map((replacementOption) => ({
                ...
                fix(fixer) {
                  // Handle special case for "Remove class" suggestion
                  if (replacementOption.startsWith('Remove class')) {
                    // Just remove the class from the string
