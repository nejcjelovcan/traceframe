...
export function parseOKLCH(oklch: string): OKLCHColor {
  const match = oklch.match(/oklch\(([0-9.]+%?)\s+([0-9.]+)\s+([0-9.]+)(?:\s*\/\s*([0-9.]+))?\)/)
 ...
 */
export function adjustLightness(color: string | OKLCHColor, amount: number): string {
 ...
 * @returns New OKLCH color string
 */
export function adjustChroma(color: string | OKLCHColor, amount: number): string {
  const parsed = typeof color === 'string' ? parseOKLCH(color) : color
 ...
 * @returns New OKLCH color string
 */
export function rotateHue(color: string | OKLCHColor, degrees: number): string {
  const parsed = typeof color === 'string' ? parseOKLCH(color) : color
 ...
 */
export function setAlpha(color: string | OKLCHColor, alpha: number): string {
  const parsed = typeof color === 'string' ? parseOKLCH(color) : color
 ...
 */
export function isLight(color: string | OKLCHColor, threshold = 0.6): boolean {
  const parsed = typeof color === 'string' ? parseOKLCH(color) : color
 ...
 */
export function createColorVariable(name: string, color: string | OKLCHColor): string {
